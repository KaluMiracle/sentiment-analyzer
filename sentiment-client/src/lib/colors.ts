...
export function getDisasterTypeColor(disasterType: string | null): string {
  ...
  if (
    normalizedType.includes("typhoon") ||
    normalizedType.includes("storm") ||
    normalizedType.includes("bagyo")
  )
    return disasterTypeColors.Typhoon;
  if (normalizedType.includes("fire") || normalizedType.includes("sunog"))
    return disasterTypeColors.Fire;
  if (
    normalizedType.includes("volcano") ||
    normalizedType.includes("volcanic") ||
    normalizedType.includes("eruption") ||
    normalizedType.includes("bulkan")
  )
    return disasterTypeColors["Volcanic Eruptions"];
  if (
    normalizedType.includes("earthquake") ||
    normalizedType.includes("quake") ||
    normalizedType.includes("lindol")
  )
    return disasterTypeColors.Earthquake;
  if (
    normalizedType.includes("landslide") ||
    normalizedType.includes("mudslide")
  )
    return disasterTypeColors.Landslide;
