...
export async function getCoordinates(
  ...
  try {
    ...
    if (results.length === 0) {
      ...
      const alternativeQuery = encodeURIComponent(
        location.split(" ").join("+") + "+Philippines"
      );
      ...
      if (altResults.length === 0) {
        console.log(
          `❌ No results found for "${location}" using alternative search`
        );
        return null;
      }
      ...
      if (
        ...
        console.log(
          `✅ Found coordinates for "${location}" using alternative search: [${latitude}, ${longitude}]`
        );
        // Cache the result
    ...
    if (
      ...
      console.log(
        `✅ Found coordinates for "${location}" using primary search: [${latitude}, ${longitude}]`
      );
      // Cache the result
      const coordinates: [number, number] = [latitude, longitude];
      geocodingCache.set(location, coordinates);
