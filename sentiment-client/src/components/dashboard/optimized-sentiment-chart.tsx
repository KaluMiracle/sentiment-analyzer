...
export function OptimizedSentimentChart({
        ...
        datasets: [
          {
            ...
            borderColor:
              type === "line"
                ? colors
                : Array(processedData.labels.length).fill("#ffffff"),
  ...
  const getChartIcon = () => {
    switch (type) {
      ...
      case "line":
        return <LineChart className="h-8 w-8 text-blue-400" />;
      case "doughnut":
      default:
        return <PieChart className="h-8 w-8 text-blue-400" />;
    }
  };
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          ...
          >
            ...
            <p className="text-sm text-slate-500 max-w-[250px] mb-4">
              Upload dataset files to analyze sentiment distribution across
              disaster events
            </p>
