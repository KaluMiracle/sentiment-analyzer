...
export function StatusCard({
  ...
  return (
    <Card className="overflow-hidden shadow-lg border-none hover:shadow-xl transition-all duration-300 group">
      <div
        className={`h-full flex flex-col relative overflow-hidden rounded-xl`}
      >
        {/* Gradient background */}
        <div
          className={`absolute inset-0 bg-gradient-to-br ${scheme.bg} opacity-90`}
        ></div>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex justify-between items-start mb-4">
                <div
                  className={`w-12 h-12 rounded-full ${scheme.iconBg} flex items-center justify-center shadow-lg`}
                >
                  <IconComponent className={`h-6 w-6 ${scheme.iconColor}`} />
                </div>
                <p
                  className={`text-sm font-medium uppercase tracking-wider opacity-80 ${scheme.textColor}`}
                >
                  {title}
                </p>
              </div>
              ...
              {trend && (
                <div className="mt-auto pt-4">
                  <div className="flex items-center space-x-1">
                    {trend.isUpward !== null && (
                      <span
                        className={`flex items-center ${
                          trend.isUpward ? "text-green-300" : "text-red-300"
                        }`}
                      >
                        {trend.isUpward ? (
                    ...
                    {trend.isUpward === null && (
                      <span className={`text-sm ${scheme.trendColor}`}>
                        {trend.value}
                      </span>
                    )}
                    <span className={`text-xs ${scheme.trendColor}`}>
                      {trend.label}
                    </span>
                  </div>
                </div>
