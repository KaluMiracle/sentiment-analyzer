...
interface AffectedAreaProps {
  sentimentPosts: FacebookComment[];
  isLoading?: boolean;
}
  ...
  switch (type.toLowerCase()) {
    case "flood":
      return (
        <Droplets
          className="h-4 w-4"
          style={{ color: getDisasterTypeColor(type) }}
        />
      );
    case "fire":
      return (
...
export function AffectedAreasCard({
  sentimentPosts,
  isLoading = false,
}: AffectedAreaProps) {
  ...
  useEffect(() => {
    ...
    sentimentPosts.forEach((post) => {
      ...
      // Track sentiments
      const currentSentimentCount =
        locationData.sentiment.get(post.sentiment) || 0;
      ...
      if (post.disasterType) {
        const currentTypeCount =
          locationData.disasterType.get(post.disasterType) || 0;
  ...
  return (
    <div
      ...
      style={{
        maskImage:
          "linear-gradient(to bottom, transparent, black 10%, black 90%, transparent 100%)",
          ...
          {affectedAreas.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[350px] py-8">
              ...
              <p className="text-center text-sm text-slate-400">
                Upload data to see disaster impact by location
              </p>
            </div>
          ) : (
                    ...
                    <motion.div
                      ...
                      style={{
                        backgroundColor: area.disasterType
                          ...
                          : getSentimentBadgeClasses(area.sentiment).includes(
                              "red"
                            )
                          ? "#ef4444"
                          : "#3b82f6",
