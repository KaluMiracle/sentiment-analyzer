...
export function KeyEvents({
  ...
  const getSentimentBadgeText = (sentiment: string | null) => {
    ...
    switch (sentiment) {
      case "negative":
        return "Panic sentiment spike";
      case "Fear/Anxiety":
        return "Fear/Anxiety sentiment spike";
      case "negativee":
        return "Disbelief sentiment increase";
      case "positive":
        return "Resilience sentiment increase";
      case "neutral":
      default:
        return "Neutral sentiment change";
    }
  };
  ...
  const generateAutoEvents = (): AutoGeneratedEvent[] => {
    ...
    Object.entries(disasterGroups).forEach(([disasterType, posts], index) => {
      ...
      const locations = Array.from(
        new Set(posts.filter((p) => p.location).map((p) => p.location))
      );
      const locationStr =
        locations.length > 0
          ? locations.slice(0, 3).join(", ") +
            (locations.length > 3 ? ", and others" : "")
          : null;
      ...
      generatedEvents.push({
        ...
        description: `Analysis of ${
          posts.length
        } posts related to ${disasterType} ${
          locationStr ? `in ${locationStr}` : ""
        }. Dominant public sentiment: ${dominantSentiment}.`,
  ...
  const allEvents = events.length > 0 ? events : generateAutoEvents();
              ...
              return (
                <div key={event.id} className="relative pl-8">
                  ...
                  <div className="mt-3 flex flex-wrap gap-2">
                    <Badge variant={getSentimentVariant(event.sentimentImpact)}>
