...
export function SentimentTimeline({
  ...
  const availableYears =
    rawDates.length > 0
      ? Array.from(
          new Set(rawDates.map((dateStr) => getYear(parseISO(dateStr))))
        ).sort((a, b) => a - b)
  ...
  const filterDataByTimeRangeAndYear = () => {
    ...
    const yearFilteredDates = datePairs.filter((pair) =>
      selectedYears.includes(pair.year)
    );
    ...
    if (timeRange !== "year") {
      ...
      timeFilteredDates = yearFilteredDates.filter(
        (pair) =>
          isAfter(pair.date, cutoffDate) || isEqual(pair.date, cutoffDate)
      );
    }

    // Get filtered formatted dates for labels
    const filteredLabels = Array.from(
      new Set(timeFilteredDates.map((pair) => pair.formatted))
    );

    // Sort chronologically
    filteredLabels.sort((a, b) => {
      const dateA = parseISO(
        datePairs.find((pair) => pair.formatted === a)?.original || ""
      );
      const dateB = parseISO(
        datePairs.find((pair) => pair.formatted === b)?.original || ""
      );
  ...
  const getRangeDescription = () => {
    ...
    switch (timeRange) {
      case "day":
        return `Last 24 hours, ${yearsText} (${count} data point${
          count !== 1 ? "s" : ""
        })`;
      case "week":
        return `Last 7 days, ${yearsText} (${count} data point${
          count !== 1 ? "s" : ""
        })`;
      case "month":
        return `Last 30 days, ${yearsText} (${count} data point${
          count !== 1 ? "s" : ""
        })`;
      case "year":
        return `Full year${
          selectedYears.length > 1
            ? "s " + selectedYears.join(", ")
            : " " + selectedYears[0]
          ...
          <div className="flex justify-between items-center">
            <div className="flex gap-2 items-center">
              <Button
                ...
                disabled={
                  selectedYears.length !== 1 ||
                  selectedYears[0] === Math.min(...availableYears)
                }
              >
              ...
              <Button
                ...
                disabled={
                  selectedYears.length !== 1 ||
                  selectedYears[0] === Math.max(...availableYears)
                }
              >
      ...
      <CardContent className="p-5">
        {filteredData.labels.length === 0 ? (
          <div className="h-80 flex items-center justify-center text-slate-500">
            No data available for {selectedYears.join(", ")} in the selected
            time range
          </div>
        ) : (
          <div className="h-80">
