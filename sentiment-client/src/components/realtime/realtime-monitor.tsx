...
export function RealtimeMonitor() {
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const hasFilipinoPhrases =
        /\b(ang|ng|mga|sa|ko|mo|nang|para|nung|yung|at|pag|ni|si|kay|na|po|opo|din|rin|nga|ba|eh|ay|ito|iyan|iyon|dito|diyan|doon)\b/i.test(
          normalizedText.toLowerCase()
        );
      const result = await analyzeText(normalizedText);
      const detectedLanguage =
        hasFilipinoPhrases || result.post.language === "tl" ? "tl" : "en";
      ...
      if (isNonDisasterInput && !autoAnalyze) {
        toast({
          ...
          description:
            "This appears to be a non-disaster related input. For best results, please enter text about disaster situations.",
          variant: "destructive",
          duration: 5000,
        });
      } else if (!autoAnalyze) {
        toast({
          ...
          description: `Language: ${
            detectedLanguage === "tl" ? "Filipino" : "English"
          }, Sentiment: ${result.post.sentiment}`,
        });
      }
  ...
  const getProgressValue = () => {
    ...
    return (
      (analysisProgress.status.processed / analysisProgress.status.total) * 100
    );
  };
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                {analysisProgress.startTime && (
                  <span className="text-xs text-slate-500">
                    {Math.round(
                      (new Date().getTime() -
                        analysisProgress.startTime.getTime()) /
                        100
                    ) / 10}
                    s
                  </span>
        ...
        <CardHeader className="p-5 border-b border-gray-200">
          ...
          <CardDescription className="text-sm text-slate-500">
            {analyzedTexts.length === 0
              ...
              : `Showing ${analyzedTexts.length} analyzed text${
                  analyzedTexts.length !== 1 ? "s" : ""
                }`}
          </CardDescription>
        </CardHeader>
              ...
              {analyzedTexts.map((item, index) => (
                <div key={index} className="p-4 bg-slate-50 rounded-lg">
                  ...
                  <div className="mt-2 flex justify-between text-xs text-slate-500">
                    <span>
                      Confidence: {(item.confidence * 100).toFixed(3)}%
                    </span>
                  ...
                  {item.explanation &&
                    !item.explanation.includes("Fallback") && (
