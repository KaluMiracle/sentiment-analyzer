...
export function UploadProgressModal() {
  const { isUploading, uploadProgress } = useFacebookDataContext();
  ...
  const isCooldown =
    stage.toLowerCase().includes("cooldown") ||
  ...
  const cooldownTimeRemaining = (() => {
    if (isCooldown) {
      ...
      const match =
        stage.match(/Cooldown: (\d+) seconds? remaining/i) ||
            ...
            <div className="mb-4 bg-gradient-to-br from-amber-800/30 to-amber-700/30 backdrop-blur-sm rounded-xl border border-amber-600/30 overflow-hidden shadow-lg">
              <div className="p-4">
                ...
                <div className="h-3 bg-slate-800/50 rounded-full overflow-hidden">
                  <motion.div
                    ...
                    animate={{
                      width: "0%",
                      transition: {
                        duration: cooldownTimeRemaining,
                        ease: "linear",
                      },
                    }}
                  />
                ...
                <div className="mt-3 text-xs text-amber-300/70 italic text-center">
                  Required 60-second pause between batches
                  <br />
                  <span className="font-semibold">
                    Processing in batches of 30 records
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div
              className={`flex items-center gap-3 p-3 rounded-lg transition-colors ${
                isLoading
                  ? "bg-blue-900/30 text-blue-300 shadow-lg border border-blue-500/20"
            ...
            >
              ...
              <span className="text-sm font-medium">
                {isProcessing
                  ? `Processing record ${processed.toLocaleString()} of ${total.toLocaleString()}`
                  : "Processing Records"}
              </span>
              {isProcessing && (
                <Loader2 className="h-4 w-4 ml-auto animate-spin" />
