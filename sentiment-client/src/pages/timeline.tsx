...
export default function Timeline() {
  const { fbPostComments, facebookPosts } = useDisasterContext();

  // Process sentiment posts to create timeline data
  const processTimelineData = () => {
    // Skip processing if no posts
    if (!Array.isArray(fbPostComments) && !Array.isArray(facebookPosts)) {
      return {
        labels: [],
        datasets: [],
        rawDates: [],
    ...
    if (fbPostComments?.length === 0 || facebookPosts?.length === 0) {
      return {
        labels: [],
        datasets: [],
        rawDates: [],
    ...
    const sortedPosts = [...fbPostComments].sort((a, b) => {
    ...
    [...sortedPosts, ...facebookPosts].forEach((item) => {
    ...
    const sentiments = ["postive", "neutral", "negative"];
    ...
    sortedPosts.forEach((post) => {
      ...
      if (sentimentCounts[postDate] && post.sentimentLabel) {
        sentimentCounts[postDate][post.sentimentLabel] =
          (sentimentCounts[postDate][post.sentimentLabel] || 0) + 1;
      }
    });
  ...
  const timelineData = useMemo(() => processTimelineData(), [fbPostComments]);

  // Calculate if we need to auto-generate disaster events
  const shouldGenerateEvents =
    facebookPosts.length === 0 && fbPostComments.length > 0;
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-slate-500">
          {fbPostComments.length > 0
            ? `Tracking ${fbPostComments.length} data points across ${timelineData.labels.length} unique dates`
